...
**Minimum release age / cooldown windows** — blocking packages published within the last N days (typically 7). Most supply chain attacks involving typosquatting or account takeover are detected and removed from registries within hours to days. A cooldown window gives the community time to catch malicious versions before they reach production. This control can be enforced at the resolver level (pnpm, Yarn Berry, Bun, npm, uv, cargo-cooldown) or at the PR level (Dependabot, Renovate). These are alternatives, not layers to stack: a resolver-level cooldown has no security-update exception, so combining it with a Dependabot cooldown blocks automated security updates. Prefer the Dependabot cooldown for most repositories — see [Cooldown: resolver-level vs Dependabot](docs/dependabot.md#cooldown-resolver-level-vs-dependabot--pick-one).
