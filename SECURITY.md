...
- **Code-Review** — Scorecard expects every changeset to be approved by a different person than the author. This repository follows a deliberate single-maintainer policy: the owner merges their own PRs via the standard PR flow. Branch protection requires all status checks to pass, linear history, conversation resolution, no force-push, no deletion, and `enforce_admins: true` (the owner is subject to the same rules). `required_approving_review_count` is `0` — a PR is required, but no approving review (GitHub forbids approving your own PR, so requiring one would block every merge on a solo project). `dismiss_stale_reviews` is `true` so the rule tightens automatically when a second maintainer joins.
- **Branch-Protection — codeowners review not required** — a sub-warning of the above. `require_code_owner_reviews: true` would block every merge for the same reason (no self-approval). Auto-resolves when a second maintainer is added.
- **Branch-Protection — last push approval is disabled** — same root cause. `require_last_push_approval: true` blocks self-merge *even with* `required_approving_review_count: 0`, because it requires an approval on the most recent push that the pusher can't give. Auto-resolves when a second maintainer is added.
